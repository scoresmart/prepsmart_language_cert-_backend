...
// Dedicated system user ID for NIM's calendar tokens
...
serve(async (req) => {
  try {
    ...
    const clientId = Deno.env.get('NIM_GOOGLE_CLIENT_ID');
    const clientSecret = Deno.env.get('NIM_GOOGLE_CLIENT_SECRET');
    const redirectUri = `${Deno.env.get('SUPABASE_URL')}/functions/v1/nim-calendar-callback`;
    ...
    console.log('NIM token exchange status:', tokenResponse.status);
    ...
    if (upsertError) {
      console.error('Failed to store NIM tokens:', upsertError);
      return new Response(
    ...
    console.log('NIM Google Calendar tokens stored successfully');
      ...
      `<html>
        <body style="font-family: system-ui; display: flex; justify-content: center; align-items: center; height: 100vh; margin: 0; background: #f0f0f0;">
          <div style="text-align: center; padding: 40px; background: white; border-radius: 12px; box-shadow: 0 4px 6px rgba(0,0,0,0.1);">
            <h1 style="color: #22c55e;">✓ NIM's Google Calendar Connected!</h1>
    ...
    console.error('NIM callback error:', error);
