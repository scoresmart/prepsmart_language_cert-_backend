...
async function getValidAccessToken(supabase: ReturnType<typeof createClient>): Promise<string> {
  const { data: tokenData, error } = await supabase
    ...
    .select('*')
    .eq('user_id', NIM_USER_ID)
    .single();

  if (error || !tokenData) {
    throw new Error('NIM calendar not connected. Please run the auth flow first.');
  }

  let accessToken = tokenData.access_token;

  if (new Date(tokenData.token_expiry) <= new Date()) {
    console.log('NIM token expired, refreshing...');
    const refreshResp = await fetch('https://oauth2.googleapis.com/token', {
      method: 'POST',
      headers: { 'Content-Type': 'application/x-www-form-urlencoded' },
      body: new URLSearchParams({
        client_id: Deno.env.get('NIM_GOOGLE_CLIENT_ID')!,
        client_secret: Deno.env.get('NIM_GOOGLE_CLIENT_SECRET')!,
...
serve(async (req) => {
  try {
    ...
    const nimSlots = (slots || []).filter((slot: any) =>
      NIM_NAMES.includes((slot.tutors?.full_name || '').trim().toLowerCase())
    );

    if (nimSlots.length === 0) {
      return new Response(JSON.stringify({ message: 'No scheduled NIM slots found.', synced: 0 }), {
        headers: { 'Content-Type': 'application/json' },
      });
    }

    const studentIds = [...new Set(nimSlots.map((s: any) => s.student_id).filter(Boolean))];
    ...
    for (const slot of nimSlots) {
      const student = studentById.get(slot.student_id);
      const studentName = student?.name || 'Student';
      const tutorName = slot.tutors?.full_name || 'NIM';
      ...
      try {
        ...
        if (calResp.ok) {
          ...
          console.log(`Slot ${slot.id} → NIM event ${calData.id}`);
          synced++;
    ...
    return new Response(
      JSON.stringify({ total: nimSlots.length, synced, failed, results }),
      { headers: { 'Content-Type': 'application/json' } }
    );
  } catch (error: unknown) {
