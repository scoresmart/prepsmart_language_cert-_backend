...
async function getValidAccessToken(supabase: ReturnType<typeof createClient>): Promise<string> {
  const { data: tokenData, error } = await supabase
    ...
    .select('*')
    .eq('user_id', ALISHA_USER_ID)
    .single();

  if (error || !tokenData) {
    throw new Error('Alisha calendar not connected. Please run the auth flow first.');
  }
  ...
  if (new Date(tokenData.token_expiry) <= new Date()) {
    const refreshResp = await fetch('https://oauth2.googleapis.com/token', {
      ...
      body: new URLSearchParams({
        client_id: Deno.env.get('ALISHA_GOOGLE_CLIENT_ID')!,
        client_secret: Deno.env.get('ALISHA_GOOGLE_CLIENT_SECRET')!,
...
serve(async (req) => {
  try {
    ...
    const alishaSlots = (slots || []).filter((slot: any) =>
      ALISHA_NAMES.includes((slot.tutors?.full_name || '').trim().toLowerCase())
    );

    if (alishaSlots.length === 0) {
      return new Response(JSON.stringify({ message: 'No scheduled Alisha slots found.', synced: 0 }), {
        headers: { 'Content-Type': 'application/json' },
      });
    }

    const studentIds = [...new Set(alishaSlots.map((s: any) => s.student_id).filter(Boolean))];
    ...
    for (const slot of alishaSlots) {
      const student = studentById.get(slot.student_id);
      const studentName = student?.name || 'Student';
      const tutorName = slot.tutors?.full_name || 'ALISHA';
      ...
      try {
        ...
        if (calResp.ok) {
          ...
          console.log(`Slot ${slot.id} → Alisha event ${calData.id}`);
          synced++;
    ...
    return new Response(
      JSON.stringify({ total: alishaSlots.length, synced, failed, results }),
      { headers: { 'Content-Type': 'application/json' } }
    );
  } catch (error: unknown) {
