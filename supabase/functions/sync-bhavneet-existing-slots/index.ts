...
async function getValidAccessToken(supabase: ReturnType<typeof createClient>): Promise<string> {
  const { data: tokenData, error } = await supabase
    ...
    .select('*')
    .eq('user_id', BHAVNEET_USER_ID)
    .single();

  if (error || !tokenData) {
    throw new Error('Bhavneet calendar not connected. Please run the auth flow first.');
  }

  let accessToken = tokenData.access_token;

  if (new Date(tokenData.token_expiry) <= new Date()) {
    console.log('Bhavneet token expired, refreshing...');
    const refreshResp = await fetch('https://oauth2.googleapis.com/token', {
      method: 'POST',
      headers: { 'Content-Type': 'application/x-www-form-urlencoded' },
      body: new URLSearchParams({
        client_id: Deno.env.get('BHAVNEET_GOOGLE_CLIENT_ID')!,
        client_secret: Deno.env.get('BHAVNEET_GOOGLE_CLIENT_SECRET')!,
...
serve(async (req) => {
  try {
    // Simple secret key guard to prevent unauthorized triggers
    ...
    // Fetch all scheduled slots from start of today onwards
    ...
    // Filter to only Bhavneet's slots
    const bhavneetSlots = (slots || []).filter((slot: any) =>
      BHAVNEET_NAMES.includes((slot.tutors?.full_name || '').trim().toLowerCase())
    );

    if (bhavneetSlots.length === 0) {
      return new Response(JSON.stringify({ message: 'No scheduled Bhavneet slots found.', synced: 0 }), {
        headers: { 'Content-Type': 'application/json' },
      });
    }

    // Fetch student profiles
    const studentIds = [...new Set(bhavneetSlots.map((s: any) => s.student_id).filter(Boolean))];
    ...
    for (const slot of bhavneetSlots) {
      const student = studentById.get(slot.student_id);
      const studentName = student?.name || 'Student';
      const tutorName = slot.tutors?.full_name || 'Bhavneet';
      ...
      try {
        ...
        if (calResp.ok) {
          ...
          console.log(`Slot ${slot.id} → Bhavneet event ${calData.id}`);
          synced++;
    ...
    return new Response(
      JSON.stringify({ total: bhavneetSlots.length, synced, failed, results }),
      { headers: { 'Content-Type': 'application/json' } }
    );
  } catch (error: unknown) {
