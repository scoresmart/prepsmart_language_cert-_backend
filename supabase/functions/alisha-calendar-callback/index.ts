...
serve(async (req) => {
  try {
    ...
    const clientId = Deno.env.get('ALISHA_GOOGLE_CLIENT_ID');
    const clientSecret = Deno.env.get('ALISHA_GOOGLE_CLIENT_SECRET');
    const redirectUri = `${Deno.env.get('SUPABASE_URL')}/functions/v1/alisha-calendar-callback`;
      ...
      `<html>
        <body style="font-family: system-ui; display: flex; justify-content: center; align-items: center; height: 100vh; margin: 0; background: #f0f0f0;">
          <div style="text-align: center; padding: 40px; background: white; border-radius: 12px; box-shadow: 0 4px 6px rgba(0,0,0,0.1);">
            <h1 style="color: #22c55e;">✓ Alisha's Google Calendar Connected!</h1>
